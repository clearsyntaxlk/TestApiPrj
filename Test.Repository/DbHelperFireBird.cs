...
{
    public class DbHelperFireBird : IDbHelper
    {
        ...
        public delegate void FirebirdSqlChangeEventHandler(object sender, FirebirdSqlChangeEventArgs e);
        ...
        CancellationToken? cancellationToken = null)
        {
            ...
            var retryPolicy = Policy
                .Handle<FbException>()  // Specify the type of exception to handle
        ...
        /// <returns></returns>
        private static async Task<T> HandleSqlCall<T>(string connectionString, Func<FbConnection, Task<T>> func)
        {
            return await Policy
                          ...
                          .Handle<FbException>(ex => ex.ErrorCode == 1205)
                          .RetryAsync(3)
                          .ExecuteAsync(async () =>
                          {
        ...
        private static void SubscribeForChanges(string commandText, CommandType commandType, int timeOut)
        {
            try
            {
        ...
        /// <param name="timeOut"></param>
        /// <param name="onChange"></param>
        private static void Subscribe(string commandText, CommandType commandType, int timeOut)
        {
            try
            {
                // always new object
                OpenConnection();
                FbCommand comm = new FbCommand(commandText, conn)
                {
        ...
        {
            //Dependency.Stop(connectionString);
        }
        ...
        /// <param name="timeOut"></param>
        private static void StartDependencyWatcher(string connectionString, string commandText, CommandType commandType, SqlSubscribeErrorHandler sqlSubscribeError, int timeOut = 30)
        {
            try
            {
                TimeSpan pauseBetweenFailures = TimeSpan.FromSeconds(30);
        ...
        {
            using (IDbConnection cn = new FbConnection(connectionString))
            {
            ...
            string connectionString)
        {
            // Single exception type with condition
            return await Policy
              .Handle<FbException>(ex => ex.ErrorCode == 1205) // sql deadlock resource (https://sqlbak.com/academy/transaction-process-id-was-deadlocked-on-lock-resources-with-another-process-and-has-been-chosen-as-the-deadlock-victim-msg-1205)
              .RetryAsync(3)
              .ExecuteAsync(async () =>
              {
                  using (FbConnection cn = new FbConnection(connectionString))

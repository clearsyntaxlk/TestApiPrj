...
namespace Test.Repository 
{
    public class DbHelperMSSQL : IDbHelper
    {
      ...
      CancellationToken? cancellationToken = null
        ...
        /// <returns></returns>
        private static async Task<T> HandleSqlCall<T>(string connectionString, Func<SqlConnection, Task<T>> func)
        {
            return await Policy
        ...
        private static void SubscribeForChanges(string commandText, CommandType commandType, int timeOut, OnChangeEventHandler onChange)
        {
            try
            {
                ...
                retryPolicy.Execute(() =>
                {
                    Subscribe(commandText, commandType, timeOut, onChange);
                });
            }
            catch (Exception)
            {
        ...
        /// <param name="timeOut"></param>
        /// <param name="onChange"></param>
        private static void Subscribe(string commandText, CommandType commandType, int timeOut, OnChangeEventHandler onChange)
        {
            try
            {
                // always new object
        ...
        /// <param name="timeOut"></param>
        private static void StartDependencyWatcher(string connectionString, string commandText, CommandType commandType, OnChangeEventHandler onChange, SqlSubscribeErrorHandler sqlSubscribeError, int timeOut = 30)
        {
            try
            {
                TimeSpan pauseBetweenFailures = TimeSpan.FromSeconds(30);
                ...
                retryPolicy.Execute(() =>
                {
                    try
                    {
                        ...
                        SubscribeForChanges(commandText, commandType, timeOut, onChange);
                    }
                    catch (Exception ex)
                    {
            ...
            string connectionString)
        {
            // Single exception type with condition
            return await Policy
              .Handle<SqlException>(ex => ex.Number == 1205) // sql deadlock resource (https://sqlbak.com/academy/transaction-process-id-was-deadlocked-on-lock-resources-with-another-process-and-has-been-chosen-as-the-deadlock-victim-msg-1205)
              .RetryAsync(3)
              .ExecuteAsync(async () =>
              {
                  using (SqlConnection cn = new SqlConnection(connectionString))
